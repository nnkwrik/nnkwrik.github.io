...
在nettyChannel的创建过程中，无论是客户端还是服务端，最终会调用这个父类构造器
...
```java
    protected DefaultChannelPipeline(Channel channel) {
        this.channel = ObjectUtil.checkNotNull(channel, "channel");//把当前的nettyChannel保存
...
`io.netty.channel.ChannelHandlerContext`接口,pipeline中每一个节点都是这个接口的实现,包括刚才的head和tail.

```java
public interface ChannelHandlerContext extends 
                                        AttributeMap,	//可以存储属性
                                        ChannelInboundInvoker,	//用于事件传播(读)
                                        ChannelOutboundInvoker {//用于事件传播(写)
	//当前节点属于那个channel
    Channel channel();
...
//..
        ...
        public void exceptionCaught(ChannelHandlerContext ctx, Throwable cause) throws Exception {
            onUnhandledInboundException(cause);//对异常的处理,仅是通过loggr输出warn
        }
        @Override
...
看一下`addFirst0`的步骤.实际就是把它插入到双向链表中tail的前面
...
```java
    private void callHandlerAdded0(final AbstractChannelHandlerContext ctx) {
        ctx.handler().handlerAdded(ctx);//实际就是调用ChannelInitializer的handlerAdded方法
        ctx.setAddComplete();	//把状态设置为已添加完毕,ADD_COMPLETE 
    }
```

ChannelInitializer的handlerAdded方法具体做了什么?

```java
    @Override
    public void handlerAdded(ChannelHandlerContext ctx) throws Exception {
        if (ctx.channel().isRegistered()) {	//已被添加到pipeline
            initChannel(ctx);
        }
    }
---
    private boolean initChannel(ChannelHandlerContext ctx) throws Exception {
        if (initMap.putIfAbsent(ctx, Boolean.TRUE) == null) { // Guard against re-entrance.
            //还未被执行过
            try {
                //这个initChannel就是用户代码中重载的那个方法
                initChannel((C) ctx.channel());
            } finally {
                remove(ctx);//执行完成后通过remove把这个节点进行删除
            }
            return true;
        }
        return false;
