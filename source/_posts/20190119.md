...
PoolThreadCache中同时存在Arena和Cache,分配内存时优先在Cache上进行分配
...
- **用于small的PoolSubpage[]**
  之前说过,如果想从一个Page分出某个大小的SubPage时,会把整个Page都划分成Page/SubPage个SubPage.之后这个Page都用于分配这个大小的SubPage.也就是说这个Page = SubPage[]

  这里的PoolSubpage[]的元素本身就是用于划分成SubPage的Page,也就是SubPage[]
  也就是说,PoolSubpage[]的元素用于标示可以划分成X大小的SubPage的Page

  所以一个PoolSubpage会包括以下信息:

  - 这些个SubPage[]是在那个chunk上划分
  - SubPage[]中哪些已经被分配出去了
  - 这个PoolSubpage可以划分的SubPage的大小是多少
...
1. 尝试在现有的chunk上分配,就是之前图中的ChunkList
2. 如果没有chunk,通过JDK获取一段内存,创建成一个chunk进行内存分配
3. 初始化`PooledByteBuf`